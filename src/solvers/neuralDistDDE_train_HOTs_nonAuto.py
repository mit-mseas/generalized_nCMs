...
class process_DistDDE_IC:
    ...
    def __init__(self, z, aux_model, extra_channel_func, t_lowerlim, t_upperlim, t_start = np.array([0.])):
        self.z = z
        self.batch_size = z(0.).shape[0]
        self.g = aux_model
        self.extra_channel_func = extra_channel_func
    ...
    def convert_to_numpy(self, t):
        return np.stack([self.g(self.z(t[i]), self.extra_channel_func(t[i] + self.t_start), flag_fwd=1).numpy() for i in range(len(t))], axis=-1)
...
class ncm_adj_eqn:
    ...
    @tf.function
    def calc_jac_non_mark_nn(self, z_stack_deriv, extra_channels):
        with tf.GradientTape() as tape:
            tape.watch(z_stack_deriv)
            g_z_stack_deriv = self.g(z_stack_deriv, extra_channels)
    ...
    def __call__(self, lam_mu, t, tau, zy, t_start):
        ...
        dg_dz_stack_deriv = self.calc_jac_non_mark_nn(dz_dx_stack_deriv, self.model.extra_channels(t + t_start)) # Hopefully should be diagonnal
...
class grad_train_var:
    ...
    @tf.function
    def calc_jac(self, z, extra_channels_t, extra_channel_t_minus_tau):
        with tf.GradientTape() as tape:
            tape.watch(self.model.trainable_weights)
            h = self.func.call_non_mark_nn_part(z, extra_channels_t, extra_channel_t_minus_tau)
        dh_dp = tape.jacobian(h, self.model.trainable_weights, experimental_use_pfor=False)
        
        return dh_dp
    
    @tf.function
    def calc_jac_direct_nn(self, z, extra_channel):
        with tf.GradientTape() as tape:
            tape.watch(self.model.trainable_weights)
            h = self.model(z, extra_channel)
        dh_dp = tape.jacobian(h, self.model.trainable_weights, experimental_use_pfor=False)
    ...
    def lam_dfdp(self, t):
        ...
        if self.flag:
            if self.mark_flag:
                ...
                dh_dp = self.calc_jac_direct_nn(input, self.func.extra_channels(t+self.t_start))
                
            else:
                input = self.func.process_input(self.z, t, self.tau, flag_fwd=1)
                dh_dp = self.calc_jac(input, self.func.extra_channels(t+self.t_start), self.func.extra_channels(t+self.t_start-self.tau))
            lam_t = self.lam(t)
        else:
            input = self.func.compute_derivatives(self.z(t), flag_fwd=1)
            dh_dp = self.calc_jac_direct_nn(input, self.func.extra_channels(t+self.t_start))
...
class train_nDistDDE:
    ...
    def evaluate(self, args, eval_true_z, eval_true_z0, eval_val_true_z, epoch, t):
        ...
        process_true_z0 = process_DistDDE_IC(eval_true_z0, self.func.call_nn_part_indiv, self.func.extra_channels, t_lowerlim = t[0] - self.args.tau, t_upperlim = t[0])
    ...
    def frwd_pass(self, batch_z0, batch_t0, batch_t, batch_z, batch_t_start):
        ...
        batch_zy0 = process_DistDDE_IC(create_interpolator(batch_z0, batch_t0), self.func.call_nn_part_indiv, self.func.extra_channels, t_lowerlim = batch_t0[-1] - self.args.tau, t_upperlim = batch_t0[-1], t_start = batch_t_start)
