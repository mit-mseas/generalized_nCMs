...
"""
class kdv_burg_args(solver_arguments):
    
    def __init__(self, T, nt, x_left = -1, x_right = 1, Nx = 10, T_start = 0., Re = 250, dleft_bv_dt = 0, dright_bv_dt = 0, dbc_l = 1, nbc_l = 0, dbc_r = 1, nbc_r = 0, max_deriv = 2, acc = 2, acc_advec = 1, eta_1 = 1.2, eta_2 = 0.8, x_1 = -6.0, x_2 = -2.0, disp_term = 1, skip_bnd = 0):
...
#         self.vander_dxx = self.full_vander_dxx[1:-(self.args.max_deriv - 1), ]
        ...
        self.full_vander_dx_upwind = self.deriv_obj.upwind(self.grid.x_grid, acc = min(min(self.args.acc, 4), self.args.acc_advec))
...
#         self.vander_dx_advec = self.deriv_obj.vander(self.grid.x_grid, m=1, acc=self.args.acc_advec)[self.args.max_deriv - 1:-(self.args.max_deriv - 1), ]

        self.vander_dx_advec = self.deriv_obj.upwind(self.grid.x_grid, acc = self.args.acc_advec)[self.args.max_deriv - 1:-(self.args.max_deriv - 1), ]
    ...
    def rhs_int(self, t, u_t, u_t_int):
        ...
        u_x_int = np.einsum('ab, cb -> ca', self.vander_dx_advec, u_t)
        u_xx_int = np.einsum('ab, cb -> ca', self.vander_dxx, u_t)
        
        u_xxx_int = np.einsum('ab, cb -> ca', self.full_vander[3][self.args.max_deriv - 1:-(self.args.max_deriv - 1), ], u_t)
        
        du_dt_int = - 1. * np.einsum('ab, ab -> ab', u_t_int, u_x_int) - self.args.disp_term * (u_xxx_int + 5. * np.einsum('ab, ab -> ab', u_t_int, u_x_int)) #+ self.args.nu * u_xx_int
        
        return du_dt_int
    
    def jac(self, u_stack, t, t_start = np.array([0.])):
        
        rhs_jac = [tf.linalg.diag(-6. * u_stack[:, :, 1])]
        rhs_jac.append(tf.linalg.diag(-6. * u_stack[:, :, 0]))
        rhs_jac.append(tf.zeros(rhs_jac[0].shape))
        rhs_jac.append(tf.linalg.diag(-self.args.disp_term * np.ones(u_stack[:, :, 0].shape)))
        
        for i in range(4, self.args.max_deriv + 1):
            rhs_jac.append(tf.zeros(rhs_jac[0].shape))
