...
class custom_loss():
    ...
    def __call__(self, true_y, pred_y):
        loss = tf.abs(pred_y - true_y + tf.keras.backend.epsilon())
...
grid_obj_hf = fdgen.grid(args.args_eval_hf)
true_u0_hf = initial_cond(grid_obj_hf.x_grid_real, args.args_eval_hf)
deriv_obj = fdgen.deriv(args.args_eval_hf, grid_obj_hf)
norm_eval = []
norm_eval.append(tf.constant(np.max(np.abs(np.squeeze(tf.cast(true_u0_hf(0.), tf.float64).numpy())))))

for i in range(1, args.args_eval_hf.max_deriv+1):
